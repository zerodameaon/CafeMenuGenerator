...
1. **`sys._MEIPASS` resolves to `Contents/Frameworks`, not
   ...
   brightsign_help images) get mirrored into *both*. `app.py` finds its
   fonts/help images via `sys._MEIPASS`, but finds Chromium via
   `sys.executable` → `Contents/Resources/ms-playwright` (see 3). If
   `PLAYWRIGHT_BROWSERS_PATH` points anywhere else, Playwright fails at
...
2. **Chromium can't go through `--add-data` at all.** PyInstaller
   ...
   the bundle as a plain post-build shell step.
3. **Chromium lives in `Contents/Resources`, and the app is re-signed after
   it's copied in (fixed 2026-09-24).** Until then Chromium went into
   `Contents/Frameworks` and the app was never re-signed — and copying
   files into an already-signed app invalidates its signature. Locally
   that's invisible (nothing is quarantined), but every copy downloaded
   from GitHub failed Gatekeeper as **"is damaged and can't be opened"**,
   with no way past it except `xattr -cr` in Terminal. Two things were
   needed:
   - Chromium can't be under `Frameworks`: codesign treats everything
     there as code that must itself be signed, and Chromium's folder
     also holds plain files (e.g. `.js` resources), so even a shallow
     re-sign fails ("code object is not signed at all … content.js").
     Under `Resources` its files are just hashed into the outer seal.
   - A **shallow** `codesign --force --sign -` of the outer app after the
     copy. `--deep` still chokes on Chromium's nested bundles — don't use
     it. `build_app.sh` now runs `codesign --verify --deep --strict` and
     refuses to finish if it fails.
   Result, verified by zipping, unzipping, and quarantine-flagging a copy:
   the signature stays valid and Gatekeeper's verdict is the ordinary
   "rejected" for an ad-hoc app ("Apple could not verify…" → System
   Settings → Privacy & Security → **Open Anyway**) instead of "damaged".
   **Correction to an earlier note:** Playwright's Chromium is *not*
   Google-signed — it's only ad-hoc (linker) signed. So on a downloaded
   copy it can still be refused while it carries the quarantine flag, even
   after the app itself is approved; `app.py` clears that flag on its own
   `Resources/ms-playwright` at startup (the flag isn't part of the
   signature, so this is safe). That's best effort — it can't work if
   macOS runs the app from a read-only "translocated" copy — so the
   README's one-line `xattr -cr` stays the recommended first-launch step.
   **Verified on a second Mac (2026-09-24):** the re-uploaded v0.2.0
   download opened via System Settings → Privacy & Security → Open
   Anyway, with no Terminal step, and the app worked. Removing the
   Gatekeeper prompt entirely would need a paid Apple Developer ID +
   notarization.
